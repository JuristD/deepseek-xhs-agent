...
async def render_markdown_to_cards(md_file: str, output_dir: str, style_key: str = "purple"):
    ...
    print(f"\n[INFO] 开始渲染: {md_file}")
    print(f"[INFO] 使用样式: {STYLES[style_key]['name']}")
    ...
    async with async_playwright() as p:
        ...
        try:
            for i, content in enumerate(processed_cards, 1):
                print(f"  [PHOTO] 生成卡片 {i}/{total_cards}...")
                ...
                print(f"  [OK] 已生成: {card_path}")
        
        finally:
            await browser.close()
    
    print(f"\n[SPARKLE] 渲染完成！共生成 {total_cards} 张卡片，保存到: {output_dir}")
    return total_cards
...
示例:
  ...
  python render_xhs_v2.py --list-styles
    ...
    if not os.path.exists(args.markdown_file):
        print(f"[X] 错误: 文件不存在 - {args.markdown_file}")
        sys.exit(1)
