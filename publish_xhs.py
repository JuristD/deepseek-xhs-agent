...
def load_cookie() -> str:
    ...
    if not cookie:
        print("[X] 错误: 未找到 XHS_COOKIE 环境变量")
...
def validate_cookie(cookie_string: str) -> bool:
    ...
    if missing:
        print(f"⚠ Cookie 可能不完整,缺少字段: {', '.join(missing)}")
